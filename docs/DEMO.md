...
Ensure all three tiers are running locally:
- **ML Microservice**: `http://localhost:8000` (run `cd ml_model && python main.py` or `uvicorn main:app --port 8000`)
- **Backend Orchestrator**: `http://localhost:3001` (run `cd backend && node index.js`)
- **Frontend UI**: `http://localhost:3000` (run `cd frontend && npm run dev`)
...
2. Click **Evaluate Thesis**.
3. **Observe**: The resulting contract is marked as `[ANALYZE]` and the Agent War Room correctly spun up only a subset of agents (e.g., Portfolio Risk, Macro & Sector) rather than all agents.
...
## STEP 5: Test the Machine Learning Sentiment Microservice
1. Ensure the ML microservice is running on port `8000`.
2. On the **Decision Firewall** page, submit a bullish thesis: *"I want to buy TSLA because earnings are accelerating and EV adoption is surging."*
3. **Observe**: In the Evaluation Complete screen, the header displays the **ML Sentiment** badge: `Positive (98.45%)` alongside a detailed probability distribution bar (Negative %, Neutral %, Positive %).
4. Navigate to the **Agent War Room**: notice the **ML Sentiment Engine** agent is active, contributing transformer-driven NLP scores to the multi-agent consensus.
